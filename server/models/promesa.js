var entity_name = require('../data/promesa').entity_name
...
function setNew(d) {
  return new Promise(function (resolve, reject) {
    ...
    VALUES
    (${helpers.arguments(data, ', ', '@', false)});
    ${ helpers.getIdType(data) == 'string' ? `SELECT '${new_uuid}' AS ${helpers.getIdName(data)}` : `SELECT IDENT_CURRENT('${entity_name}') AS ${helpers.getIdName(data)}` }
    `
